...
#| include: false
DATEN <- tidycomm::WoJ |>
  sjlabelled::var_labels(reach = "Reichweite", employment = "Anstellung")
```
...
```{r}
DATEN |>
  tidycomm::knit_crosstab(reach, employment)
```

```{r}
DATEN |> tidycomm::knit_crosstab(reach, employment, percent = FALSE)
```
...
model <- lm(
  autonomy_selection ~ work_experience +
    trust_government +
    ethics_1 +
    ethics_2 +
    ethics_3 +
...
reg_tab <- tidycomm::regress(model, check_multicollinearity = FALSE, CIs = TRUE)
reg_tab |>
